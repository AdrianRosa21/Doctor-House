...
$sql = "SELECT id_doctor, nombre, apellido, numero_telefono, correo_electronico, usuario FROM doctor WHERE usuario = '$usuario'";
...
if ($result->num_rows > 0) {
    ...
    // Consultar la especialidad del doctor
    $sql1 = "SELECT especialidad.nombre 
             FROM especialidad
             INNER JOIN pertenece ON especialidad.id_especialidad = pertenece.id_especialidad
             INNER JOIN doctor ON pertenece.id_doctor = doctor.id_doctor
...
$imagenPerfil = 'uploads/png-transparent-user-profile-computer-icons-profile-heroes-black-silhouette-thumbnail.png'; 


// Consultar la última imagen subida para el doctor específico
$sql = "SELECT ruta FROM fotos_perfil WHERE id_doctor = $id_doctor ORDER BY fecha_subida DESC LIMIT 1";
$result = $conn->query($sql);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['foto'])) {
    ...
    if (in_array($tipoArchivo, $tiposPermitidos)) {
        ...
        if (move_uploaded_file($rutaTemporal, $rutaDestino)) {
            // Guardar la ruta de la imagen y el id_doctor en la base de datos
            $sql = "INSERT INTO fotos_perfil (ruta, id_doctor) VALUES ('$rutaDestino', $id_doctor)";
  ...
  <div class="main-content">
    ...
    <img src="<?php echo $imagenPerfil; ?>" alt="Foto de Perfil" class="perfil">
      ...
      <div class="profile-info">
        ...
        <h3>Doctor: <?php echo htmlspecialchars($doctor['nombre'])." ".  htmlspecialchars($doctor['apellido']);  ?> </h3>
        <p>Especialidad: <?php echo htmlspecialchars($resultado1['nombre']); ?></p>
          <p>Correo Electrónico: <?php echo htmlspecialchars($doctor['correo_electronico']); ?></p>
          <p>Usuario: <?php echo htmlspecialchars($doctor['usuario']); ?></p>
